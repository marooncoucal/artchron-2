...
export default function SortSections1({
  ...
  return (
    <div className="h-full">
      ...
      {/* Сетка низ */}
      <div className="relative -z-5 w-[357px] h-[256px] border border-gray-300 bg-white select-none mt-2">
        {[1, 2].map((i) => (
          <div
            key={`v2-${i}`}
            className="absolute top-0 bottom-0 w-[1px] bg-gray-300"
      ...
      {/* Кнопка Проверить */}
      <ButtonCheck inPlaceCount={inPlaceCount} images={images} />
    </div>
  )
}
