 ...
 if (!Array.isArray(imgs) || imgs.length === 0) return;
  ...
  for(let x = 0;x < width; x++){
    for(let y = 0; y < height; y++){
      ...
      gridInitPos.push({initX: imgs[index].targetX, initY: imgs[index].targetY})
    }
  }
...
export default function PuzzleBoard({ images, imageWidth, imageHeight, width, height }) {
  ...
  return (
    ...
    {[1, 2].map(i => (
      <div key={`v2-${i}`} className="absolute top-0 bottom-0 w-[1px] bg-gray-300" style={{ left: `${i * 119}px` }} />
    ))}
    {[1, 2, 3].map(i => (
      <div key={`h2-${i}`} className="absolute left-0 right-0 h-[1px] bg-gray-300" style={{ top: `${i * 64}px` }} />
    ))}
  </div>
      ...
      <div className="mt-10 flex">
        ...
        type="button"
          ...
          className={`
            ...
            ${checkStatus === "correct" ? "bg-[#57DE75] border-[#57DE75]" : ""}
            ${checkStatus === "wrong" ? "bg-[#E46F2B] border-[#E46F2B]" : ""}
...
function PuzzlePiece({ targetX, targetY, children, initX, initY, imageWidth, imageHeight, onInPlace }) {
  ...
  function onStopDrag() {
    if (dragParams.current.drag) {
      ...
      let d = distance(dragParams.current.x, dragParams.current.y, targetX, targetY)
      if (d < 10) {
